...
class IndexTTS2:
    def __init__(
        ...
        if device is not None:
            ...
            self.device = "mps"
            # Enable FP16 by default on MPS to reduce memory usage
            self.use_fp16 = True if use_fp16 is None else use_fp16
            self.use_cuda_kernel = False
            # Disable MPS high watermark to allow fuller memory use
    ...
    def _load_and_cut_audio(self,audio_path,max_audio_length_seconds,verbose=False,sr=None):
        ...
        audio = torch.tensor(audio).unsqueeze(0).float()  # Keep on CPU as float32
                    ...
                    with torch.no_grad():
                        try:
                            ...
                            if "MPS backend out of memory" in str(e) and self.device == "mps":
                                print(">> OOM on MPS; falling back to CPU for this step.")
                                self.device = "cpu"
                                # Move models to CPU as fallback
                                ...
                                self.emo_matrix = self.emo_matrix.to("cpu")
                                self.spk_matrix = self.spk_matrix.to("cpu")
                                # Retry on CPU
                                emovec = self.gpt.merge_emovec(
                                    spk_cond_emb.to("cpu"),
                                    emo_cond_emb.to("cpu"),
                                    torch.tensor([spk_cond_emb.shape[-1]], device="cpu"),
                                    torch.tensor([emo_cond_emb.shape[-1]], device="cpu"),
                                    alpha=emo_alpha
                                )
                                if emo_vector is not None:
                                    emovec = emovec_mat.to("cpu") + (1 - torch.sum(weight_vector.to("cpu"))) * emovec
                                codes, speech_conditioning_latent = self.gpt.inference_speech(
                                    spk_cond_emb.to("cpu"),
                                    text_tokens.to("cpu"),
                                    emo_cond_emb.to("cpu"),
                                    cond_lengths=torch.tensor([spk_cond_emb.shape[-1]], device="cpu"),
                                    emo_cond_lengths=torch.tensor([emo_cond_emb.shape[-1]], device="cpu"),
                                    emo_vec=emovec,
                                    do_sample=True,
                                    top_p=top_p,
                                    top_k=top_k,
